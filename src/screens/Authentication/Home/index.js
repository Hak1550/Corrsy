...
const Home = () => {
  ...
  const getSubject = (id, course_id) =>
    dispatch(getSubjectsRequest(id, course_id));
  const getLesson = id => {
    navigate('Lesson');
  ...
  const EachCourse = ({item}) => (
    <TouchableOpacity
      key={item?._id}
      style={styles.eachCourse(item?.isSelected)}
      activeOpacity={0.1}
      onPress={() => getSubject(item?.subject?._id, item?._id)}>
      <SmartImage
        source={{uri: item?.subject?.subject_icon}}
        style={styles.eachCourseImg}
      />
      <CustomText text={item?.subject?.Subject_name} style={FONTS.Bold14} />
    </TouchableOpacity>
  );
  const EachSubject = ({item}) => (
    <TouchableOpacity
      key={item?._id}
      style={styles.eachSubject}
      activeOpacity={1}>
      <SmartImage
        source={{uri: item?.chapter[0]?.chapter_icon}}
        style={styles.eachSubjectImg}
      />
  ...
  const EachLesson = ({item}) => (
    <TouchableOpacity
      ...
      style={styles.eachSubject}
      activeOpacity={0.1}
      onPress={() => getLesson(item?._id)}>
      <SmartImage source={images.abc} style={styles.eachSubjectImg} />
      <CustomText
        text={`Lesson ${item?.lessonNumber}`}
        style={[FONTS.Regular13, {color: Colors.whiteThemeColor}]}
      />
      <CustomText
        text={item?.title}
        style={[FONTS.Bold12, {textAlign: 'center'}]}
      />
